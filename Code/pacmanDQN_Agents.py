...
class PacmanDQN(game.Agent):
    def __init__(self, args):
        ...
        quantiles = [.05, .5, .95]
        self.qnet = DQN(self.params, quantiles)
    ...
    def final(self, state):
        ...
        self.currentAvg += self.ep_rew
        self.allAvg  += self.ep_rew
        if (self.numeps + 1) % 100 == 0:
            print("\n")
            print("Completed", (self.numeps + 1) , "out of", self.params['num_training'], "training episodes")
            self.currentAvg = self.currentAvg / 100
            print("Average Rewards over all training", (self.allAvg / (self.numeps + 1)))
            print("Average Rewards for last 100 episodes:", self.currentAvg)
            endTime = time.time()
            current_time = endTime - self.beginTime
